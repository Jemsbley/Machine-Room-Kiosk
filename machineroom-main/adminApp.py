...
from keypad import globalMachines, fillmachines, checkServer
...
class AdminScreen(Screen):
    ...
    def killswitch(self):
        ...
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("call switch")
                connection.commit()
    
    def killswitch_color(color):
        if color == 'root.red':
            return 'root.gray'
        else:
            return 'root.red'
    ...
    def toggleColorKs(self, currColor):
        if currColor == [1,0,0,1]:
            self.ids['kill'].background_color = [0,1,0,1]
        else:
            self.ids['kill'].background_color = [1,0,0,1]
